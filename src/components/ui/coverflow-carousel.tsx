...
export function CoverflowCarousel({
  ...
  falloff = 0.56,
  fade = 0.1,
  cardWidth = "clamp(260px, 36vw, 420px)",
  gap = 0.05,
  loop = true,
  showCaption = false,
  ...
  return (
    ...
    >
            ...
            {slides.map((slide, index) => {
              const CardContent = (
                <>
                  ...
                  />
                  {slide.title && (
                    <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent p-6 flex flex-col justify-end">
                      <span className="font-mono text-xs uppercase tracking-widest text-primary font-bold">
                        {slide.subtitle || "Design"}
                      </span>
              ...
              return (
                <div
                  ...
                  className={cn(
                    "absolute left-1/2 top-0 aspect-square overflow-hidden rounded-2xl bg-muted shadow-2xl will-change-transform border border-border/40 cursor-pointer",
                    cardClassName,
                  )}
        ...
        {showNavigation && (
          <>
            <button
              ...
              onClick={() => nudge(-1)}
              className="absolute left-3 top-1/2 z-[200] -translate-y-1/2 rounded-full bg-background/80 p-3 text-foreground backdrop-blur border border-border shadow-lg transition hover:bg-background hover:scale-110 active:scale-95"
            >
            ...
            <button
              ...
              onClick={() => nudge(1)}
              className="absolute right-3 top-1/2 z-[200] -translate-y-1/2 rounded-full bg-background/80 p-3 text-foreground backdrop-blur border border-border shadow-lg transition hover:bg-background hover:scale-110 active:scale-95"
            >
      ...
      {showCaption && active?.title && (
        ...
        >
          <p className="text-xl font-display uppercase tracking-tight text-foreground">
            {active.title}
          </p>
          {active.subtitle && (
          ...
          {active.href && (
            <a
              ...
              className="mt-4 inline-flex items-center gap-2 rounded-full bg-foreground px-5 py-2 text-xs font-mono uppercase tracking-wider text-background transition hover:opacity-90 shadow-md"
            >
              Visit Live Site ↗
            </a>
          )}
          {active.meta && active.meta.length > 0 && (
            <dl className="mt-6 w-full max-w-xs text-xs font-mono divide-y divide-border/40 border-y border-border/40 py-2">
              {active.meta.map((row) => (
        ...
        <div className="mt-6 flex items-center justify-center gap-2">
          {slides.map((_, index) => (
            <button
              ...
              onClick={() => goTo(index)}
              className={cn(
                "h-1.5 transition-all duration-300 rounded-full bg-foreground",
